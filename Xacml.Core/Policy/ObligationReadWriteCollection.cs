	...
	public class ObligationReadWriteCollection : CollectionBase 
	{
		...
		public void WriteDocument(XmlWriter writer)
		{
			...
			if( this.Count > 0 )
			{
				...
				{
					writer.WriteStartElement(PolicySchema1.ObligationElement.Obligation);
					writer.WriteAttributeString(PolicySchema1.ObligationElement.ObligationId, oObligation.ObligationId);
					writer.WriteAttributeString(PolicySchema1.ObligationElement.FulfillOn, oObligation.FulfillOn.ToString());
					foreach(AttributeAssignmentElementReadWrite oAttribute in oObligation.AttributeAssignment)
					{
						writer.WriteStartElement(PolicySchema1.ObligationElement.AttributeAssignment);
						writer.WriteAttributeString(PolicySchema1.AttributeAssignmentElement.AttributeId,oAttribute.AttributeId);
						writer.WriteAttributeString(PolicySchema1.AttributeAssignmentElement.DataType, oAttribute.DataTypeValue);
