...
namespace Xacml.Core.Policy
{
	...
	public class ResourcesElementReadWrite : TargetItemsBaseReadWrite
	{
		...
		public ResourcesElementReadWrite( XmlReader reader, XacmlVersion version ) : 
			base( reader, PolicySchema1.TargetElement.Resources, PolicySchema1.ResourceElement.AnyResource, PolicySchema1.ResourceElement.Resource, version )
		{
		}
		...
		{
			writer.WriteStartElement(PolicySchema1.TargetElement.Resources);
			if(this.IsAny)
			{
				writer.WriteElementString(PolicySchema1.ResourceElement.AnyResource, string.Empty);
			}
			else
			{
				foreach(ResourceElementReadWrite oResource in this.ItemsList)
				{
					writer.WriteStartElement(PolicySchema1.ResourceElement.Resource);
					foreach(ResourceMatchElementReadWrite oItem in oResource.Match)
					{
						writer.WriteStartElement(PolicySchema1.ResourceElement.ResourceMatch);
